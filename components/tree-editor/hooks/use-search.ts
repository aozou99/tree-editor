...
export function useSearch({ tree, nodeTypes }: UseSearchProps, setTree: (tree: TreeNode[]) => void): UseSearchReturn {
  // 検索機能用の状態
  ...
  const handleSelectSearchResult = (index: number) => {
    if (index >= 0 && index < searchResults.length) {
      ...
      // パスに含まれるノードを展開
      // setTree(expandNodesInPath(tree, result.path))
    }
  }
  ...
  const handleOpenDetailModal = (result: SearchResult) => {
    ...
    // 注意: この関数は外部から渡されたコールバックを呼び出す必要があります
    // 詳細モーダルの表示はこの関数の呼び出し元で行う必要があります
    return result.node
  }
  ...
  const handleSearchKeyDown = (e: React.KeyboardEvent) => {
    ...
    console.log(`キー押下: ${e.key}, 現在のインデックス: ${selectedResultIndex}, 検索結果数: ${searchResults.length}`)
