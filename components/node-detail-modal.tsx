...
import { isBase64Image } from "./tree-editor"
import type { TreeNode, CustomField, NodeType } from "./tree-editor"
              ...
              {isEditing ? (
                <div className="flex gap-2 items-center">
                  <div className="flex-1 border rounded-md p-2 flex items-center">
                    <div className="w-8 h-8 flex items-center justify-center">{renderIconPreview(displayIcon)}</div>
                    {!displayIcon && (
            ...
            <div>
              ...
              <div className="space-y-2">
                {editedNode.customFields && editedNode.customFields.length > 0 ? (
                    ...
                    {isEditing ? (
                      <>
                        {editedNode.customFields.map((field) => (
                          <div key={field.id} className="grid grid-cols-[1fr_auto_2fr] gap-2 items-start">
                            <Input
                              value={field.name}
                            ...
                            <Select
                              ...
                              onValueChange={(value) =>
                                updateCustomFieldType(
                                  field.id,
                                  value as "text" | "textarea" | "link" | "youtube" | "image" | "audio",
                                )
                              }
                              disabled={!!currentNodeType}
                            >
                            ...
                            {field.type === "textarea" ? (
                              ...
                              <div className="flex flex-col w-full">
                                <Input
                                  ...
                                  className={urlValidationErrors[field.id] ? "border-red-500" : ""}
                                />
                                {urlValidationErrors[field.id] && (
                                  <p className="text-red-500 text-xs mt-1">{urlValidationErrors[field.id]}</p>
                                )}
                              </div>
                            ) : field.type === "youtube" ? (
                              <div className="flex flex-col w-full">
                                  ...
                                  <div className="mt-1">
                                    {field.value && (
                                      <div className="flex items-center gap-2">
                                        ...
                                        <audio
                                          ref={(el) => (audioRefs.current[field.id] = el)}
                                          src={field.value}
                  ...
                  <div className="text-center py-4 text-sm text-muted-foreground">カスタムフィールドはありません</div>
                )}
              </div>
            </div>
          </div>
