...
function TreeEditor() {
  ...
  const handleOpenDetailModal = (result: SearchResult) => {
    ...
    const expandNodes = (nodes: TreeNode[]): TreeNode[] => {
      return nodes.map((node) => {
        if (result.path.some((pathNode) => pathNode.id === node.id)) {
          // このノードがパスに含まれる場合は展開
          return {
            ...node,
            isExpanded: true,
        ...
        } else if (node.children.some((child) => result.path.some((pathNode) => pathNode.id === child.id))) {
          // 子ノードがパスに含まれる場合も展開
          return {
            ...node,
            isExpanded: true,
  ...
  const exportTreeData = () => {
    try {
      ...
      link.href = url
      link.download = `${treeTitle.replace(/\s+/g, "-").toLowerCase()}-${new Date().toISOString().split("T")[0]}.json`
  ...
  const renderNode = (node: TreeNode, depth = 0) => {
    ...
    if (
      focusMode &&
      !isNodeInHighlightedPath(node, highlightedNodeIds) &&
      !hasHighlightedDescendant(node, highlightedNodeIds)
    ) {
      return null
    }
    ...
    return (
      ...
      >
          ...
          {editingNodeId === node.id ? (
            ...
            <>
                  ...
                  {nodeIcon && (
                    ...
                    >
                      {isImageUrl(nodeIcon) || isBase64Image(nodeIcon) ? (
                        <img src={nodeIcon || "/placeholder.svg"} alt="" className="w-5 h-5 object-cover rounded-sm" />
                      ) : (
              ...
              <div className="opacity-0 group-hover:opacity-100 flex" onClick={(e) => e.stopPropagation()}>
                <Button size="icon" variant="ghost" onClick={(e) => openAddChildModal(node.id, e)} className="h-7 w-7">
                  <Plus size={16} />
                </Button>
                <Button size="icon" variant="ghost" onClick={(e) => startEditing(node, e)} className="h-7 w-7">
                  <Edit size={16} />
                </Button>
                <Button size="icon" variant="ghost" onClick={(e) => deleteNode(node.id, e)} className="h-7 w-7">
                  <Trash size={16} />
                </Button>
              </div>
            </>
        ...
        {dragOverNodeId === node.id && dragPosition === "after" && <div className="h-1 bg-primary rounded-full mt-1" />}
          ...
          <div className="ml-4">
            {node.children
              .map((childNode) => {
                ...
                if (
                  focusMode &&
                  !isNodeInHighlightedPath(childNode, highlightedNodeIds) &&
                  !hasHighlightedDescendant(childNode, highlightedNodeIds)
                ) {
                  return null
                }
  ...
  return (
    <div className="border rounded-lg p-4">
        ...
        {isEditingTitle ? (
          <div className="flex items-center w-full">
            <Input
              ...
              autoFocus
            />
            <Button size="icon" variant="outline" onClick={() => setIsEditingTitle(false)} className="h-9 w-9">
              <Save size={16} />
            </Button>
            <Button
              size="icon"
          ...
          <div className="mt-2 p-3 bg-muted/30 rounded-md text-sm">
            ...
            <div className="space-y-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground">ノード名、サムネイル、フィールド値を検索します。</p>
            ...
            <Button variant="outline" size="sm" className="mt-2 w-full" onClick={() => setIsSearchHelpOpen(false)}>
              閉じる
            </Button>
          </div>
        )}
      ...
      <Dialog open={isSampleSelectorOpen} onOpenChange={setIsSampleSelectorOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>サンプルデータを選択してください。現在のデータは上書きされます。</DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <RadioGroup
      ...
      {/* ファイル選択用のInput要素（非表示） */}
      <input type="file" style={{ display: "none" }} accept=".json" onChange={handleFileChange} ref={fileInputRef} />
    </div>
  )
}
