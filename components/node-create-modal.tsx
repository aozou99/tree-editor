            ...
            {selectedType && (
              <>
                ...
                <div>
                  ...
                  />
                  {formErrors["nodeName"] && <p className="text-red-500 text-xs mt-1">{formErrors["nodeName"]}</p>}
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2 items-center">
                    <div className="flex-1 border rounded-md p-2 flex items-center">
                      <div className="w-8 h-8 flex items-center justify-center">{renderIconPreview(nodeIcon)}</div>
                      {!nodeIcon && (
                ...
                {/* カスタムフィールド */}
                <div>
                  <Label className="text-sm font-semibold mb-2 block">{selectedType.name}のフィールド</Label>
                  <div className="space-y-3">
                    {customFields.map((field) => {
                      const fieldDef = selectedType.fieldDefinitions.find((def) => def.id === field.definitionId)
                      return (
                        <div key={field.id} className="space-y-1">
                          <Label htmlFor={`field-${field.id}`} className="text-xs flex items-center">
                            {field.name}
